...
public class NoticesDAOImpl extends GenericDAO implements NoticesDAO {
	...
	public List<NoticesDTO> getNoticesList() throws NoticesException {
		...
		sql.append("SELECT AV_ID, AV_FECHA, AV_TITULO, AV_DETALLE, TA.TA_ID, TA.TA_TIPO_AVISO FROM ");
		...
		try{
			noticesDTOsList = this.jdbcTemplate.query(sql.toString(), new RowMapper<NoticesDTO>() {
				...
				public NoticesDTO mapRow(ResultSet rs, int arg1) throws SQLException {
					...
					dto.setTitle(rs.getString("AV_TITULO"));
	...
	public void addNotices(NoticesDTO dto) throws NoticesException {
		try{
			this.jdbcTemplate.update("INSERT INTO AVISOS (AV_FECHA, AV_TITULO, AV_DETALLE, TA_ID) VALUES (NOW(),?,?,?)", 
								new Object[]{dto.getTitle(), dto.getDetail(), dto.getTypeNoticeId()});
	...
	public void editNotices(NoticesDTO dto) throws NoticesException {
		try{
			this.jdbcTemplate.update("UPDATE AVISOS SET AV_DETALLE = ?, AV_TITULO = ?, TA_ID = ? WHERE AV_ID = ?", 
								new Object[]{dto.getDetail(), dto.getTitle(), dto.getTypeNoticeId(), dto.getId()});
		}catch(DataAccessException exception){
			log.error(exception);
	...
	public NoticesDTO getNoticeById(int id) throws NoticesException {
		NoticesDTO dto = null;
		StringBuilder sql = new StringBuilder(0);
		sql.append("SELECT AV_ID, AV_TITULO, AV_FECHA, AV_DETALLE, TA.TA_ID, TA.TA_TIPO_AVISO FROM ");
