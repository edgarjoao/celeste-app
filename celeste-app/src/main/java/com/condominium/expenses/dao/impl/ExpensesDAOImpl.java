...
public class ExpensesDAOImpl extends GenericDAO implements ExpensesDAO {
	...
	public void addExpenses(ExpensesDTO dto) throws ExpensesException {
		try{
			this.jdbcTemplate.update("INSERT INTO EGRESOS (EGR_FECHA_EGRESO, EGR_IMPORTE, EGR_COMENTARIOS, CATE_ID, PROV_ID, USR_ID) VALUES (?,?,?,?,?,?)", 
							new Object[]{dto.getDate(), dto.getAmount(), dto.getComments(), dto.getExpensesItemDTO().getId(), dto.getSuppliersDTO().getProvId(), dto.getUserId()});
		}catch(DataAccessException exception){
			log.error(exception);
			ExpensesException expensesException = new ExpensesException(exception, ExpensesException.LAYER_DAO, ExpensesException.ACTION_INSERT);
			throw expensesException;
	...
	public List<ExpensesReportDTO> getExpensesReportDTO(int month, int year) throws ExpensesException {
		...
		StringBuilder sql = new StringBuilder(0);
		try{
			sql.append("SELECT CE.CATE_ID, CE.CATE_DESCRIPCION, SUM(EGR_IMPORTE) AS IMPORTE FROM EGRESOS E ");
			...
			list = this.jdbcTemplate.query(sql.toString(), new Object[]{month, year}, new RowMapper<ExpensesReportDTO>() {
				...
				public ExpensesReportDTO mapRow(ResultSet rs, int arg1) throws SQLException {
					...
					dto.setId(rs.getInt("CE.CATE_ID"));
	...
	/**
	 * Metodo para obtener el detalle de los egresos.
	 * @param id Id del tipo de Egreso
	 * @param month Mes 
	 * @param year Anio
	 */
	public List<ExpensesDetailReportDTO> getExpensesReportById(int id, int month, int year) throws ExpensesException {
		List<ExpensesDetailReportDTO> list = null;
		StringBuilder sql = new StringBuilder(0);
		try{
			sql.append("SELECT EGR_COMENTARIOS, EGR_IMPORTE FROM EGRESOS E ");
			sql.append("INNER JOIN CATALOGO_EGRESOS CE ON E.CATE_ID = CE.CATE_ID  ");
			sql.append("WHERE MONTH(EGR_FECHA_EGRESO) = ? AND YEAR(EGR_FECHA_EGRESO) = ? AND CE.CATE_ID = ? ");			
			list = this.jdbcTemplate.query(sql.toString(), new Object[]{month, year, id}, new RowMapper<ExpensesDetailReportDTO>() {
				
				public ExpensesDetailReportDTO mapRow(ResultSet rs, int arg1) throws SQLException {
					ExpensesDetailReportDTO dto = new ExpensesDetailReportDTO();
					dto.setComments(rs.getString("EGR_COMENTARIOS"));
					dto.setAmount(rs.getDouble("EGR_IMPORTE"));
					return dto;
				}
