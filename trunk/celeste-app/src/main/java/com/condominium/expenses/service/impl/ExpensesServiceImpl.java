...
public class ExpensesServiceImpl implements ExpensesService {
	...
	public List<ExpensesReportView> getExpensesReport(int month, int year) throws ExpensesException {
		...
		try{
			ExpensesConverter converter = new ExpensesConverter();
			List<ExpensesReportDTO> tmpList = this.expensesDAO.getExpensesReportDTO(month, year);			
			if(!tmpList.isEmpty()){
				for(ExpensesReportDTO dto : tmpList){
					ExpensesReportView expensesReportView = converter.convertExpensesDTOToView(dto);									
					List<ExpensesDetailReportDTO> deDtos = this.expensesDAO.getExpensesReportById(dto.getId(), month, year);
					expensesReportView.setDetailList(converter.convertExpensesRDetailDTOToViews(deDtos));
					list.add(expensesReportView);
				}
			}			
		}catch(ExpensesException e){
